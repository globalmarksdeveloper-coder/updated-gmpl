    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sg{width:100%;max-width:1000px;display:grid;grid-template-columns:5fr 8fr;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(15,23,42,0.14);border:1px solid #E2E8F0;}

        /* LEFT */
        .lp{background:linear-gradient(160deg,#0F2460 0%,#1E3A8A 60%,#1a3580 100%);padding:44px 36px;display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden;min-height:560px}
        .lp-circle1{position:absolute;top:-80px;right:-80px;width:260px;height:260px;border-radius:50%;background:rgba(59,130,246,0.2);pointer-events:none}
        .lp-circle2{position:absolute;bottom:-60px;left:-60px;width:220px;height:220px;border-radius:50%;background:rgba(16,185,129,0.12);pointer-events:none}
        .lp-tag{font-size:9.5px;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:rgba(255,255,255,0.4);font-family:'Outfit',sans-serif;position:relative;z-index:1}
        .lp-hero{position:relative;z-index:1}
        .lp-hero h2{font-family:'Outfit',sans-serif;font-size:28px;font-weight:800;color:#fff;line-height:1.18;letter-spacing:-0.02em;margin-bottom:10px}
        .lp-hero h2 span{color:#10B981}
        .lp-hero p{font-size:12.5px;color:rgba(255,255,255,0.5);line-height:1.65;max-width:260px}
        .lp-bullets{position:relative;z-index:1;display:flex;flex-direction:column;gap:8px}
        .lp-bullet{display:flex;align-items:center;gap:9px;font-size:12px;color:rgba(255,255,255,0.72)}
        .lp-dot{width:6px;height:6px;border-radius:50%;background:#10B981;flex-shrink:0}

        /* RIGHT */
        .rp{background:#F8FAFC;padding:28px 36px;display:flex;flex-direction:column;overflow-y:auto;max-height:100vh}
        .badge{margin-bottom:10px;display:inline-flex;align-items:center;gap:5px;padding:4px 11px;border-radius:20px;background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.25);font-size:10px;font-weight:600;color:#065f46;font-family:'Outfit',sans-serif;letter-spacing:0.04em;margin-bottom:12px;width:fit-content}
        .badge-dot{width:5px;height:5px;border-radius:50%;background:#10B981}
        .rp h1{font-family:'Outfit',sans-serif;font-size:21px;font-weight:800;color:#0F172A;letter-spacing:-0.02em;margin-bottom:3px}
        .rp-sub{font-size:12.5px;color:#64748B;margin-bottom:12px}

        /* Section header */
        .sec-hdr{display:flex;align-items:center;gap:8px;margin:10px 0 8px}
        .sec-line{flex:1;height:1px;background:#E2E8F0}
        .sec-txt{font-size:10px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.1em;font-family:'Outfit',sans-serif;white-space:nowrap}

        /* Roles */
        .roles{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px}
        .role-btn{display:flex;align-items:center;gap:8px;padding:9px 12px;border-radius:10px;border:1.5px solid #E2E8F0;background:#fff;cursor:pointer;font-size:12.5px;font-weight:500;color:#64748B;transition:all 0.18s;font-family:'Plus Jakarta Sans',sans-serif}
        .role-btn:hover{border-color:#1E3A8A;color:#1E3A8A;background:rgba(30,58,138,0.04)}
        .role-btn.active{border-color:#1E3A8A;background:rgba(30,58,138,0.07);color:#1E3A8A;font-weight:700}
        ...
        .field-lbl{font-size:11.5px;font-weight:600;color:#374151;font-family:'Outfit',sans-serif;letter-spacing:0.01em}
        .field-lbl span{color:#EF4444}
        .field-lbl .opt{color:#94A3B8;font-size:10px;font-weight:400}
        .inp-wrap{position:relative;display:flex;align-items:center}
        .inp-ico{position:absolute;left:11px;top:50%;transform:translateY(-50%);color:#94A3B8;display:flex;pointer-events:none;z-index:1}
        .inp{width:100%;height:40px;padding:0 12px 0 36px;border-radius:10px;border:1.5px solid #E2E8F0;background:#fff;font-size:13px;color:#0F172A;font-family:'Plus Jakarta Sans',sans-serif;outline:none;transition:all 0.18s}
        .inp:focus{border-color:#3B82F6;box-shadow:0 0 0 3px rgba(59,130,246,0.1);background:#fff}
        .inp.no-ico{padding-left:12px}
        .sel{width:100%;height:40px;padding:0 12px 0 36px;border-radius:10px;border:1.5px solid #E2E8F0;background:#fff;font-size:13px;color:#0F172A;font-family:'Plus Jakarta Sans',sans-serif;outline:none;transition:all 0.18s;cursor:pointer;appearance:none}
        .sel:focus{border-color:#3B82F6;box-shadow:0 0 0 3px rgba(59,130,246,0.1)}
        .pw-toggle{position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#94A3B8;display:flex;padding:3px}

        /* Strength bar */
        .str-bar{height:4px;border-radius:4px;background:#E2E8F0;margin-top:4px;overflow:hidden}
        ...
        .alert.ok{background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.25);color:#065f46}

        /* Submit */
        .sub-btn{width:100%;height:44px;border-radius:10px;background:#1E3A8A;color:#fff;font-family:'Outfit',sans-serif;font-size:14px;font-weight:700;border:none;cursor:pointer;letter-spacing:0.02em;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:4px;box-shadow:0 4px 14px rgba(30,58,138,0.3)}
        .sub-btn:hover:not(:disabled){background:linear-gradient(135deg,#047857 0%,#059669 100%);transform:translateY(-1px);box-shadow:0 6px 18px rgba(16,185,129,0.45)}
        ...
        .signin-row{text-align:center;font-size:12.5px;color:#64748B}
        .signin-link{color:#3B82F6;font-weight:600;text-decoration:none}
        .signin-link:hover{color:#1E3A8A}

        /* No scroll */
        html,body{height:100%;overflow:hidden}
        .sw{height:100vh;display:flex;align-items:center;justify-content:center;padding:12px;overflow:hidden}
        .sg{width:100%;max-width:1040px;height:calc(100vh - 24px);display:grid;grid-template-columns:5fr 8fr;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(15,23,42,0.14);border:1px solid #E2E8F0}
      ...
      <div className="sw">
        <div className="sg">
          ...
          <div className="lp">
            ...
            {/* Tag */}
            <div className="lp-tag">Field Operations System</div>
            ...
            <div style={{position:"relative",zIndex:1}}>
              <p style={{fontSize:10,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"rgba(255,255,255,0.35)",fontFamily:"'Outfit',sans-serif",marginBottom:10}}>Register as</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[
                  ...
                  {key:"tsc",   label:"TSC",             icon:"M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z",                                                                              color:"#F59E0B"},
                  ...
                  <div key={r.key} onClick={()=>setRole(r.key)}
                    style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"12px 8px",borderRadius:12,cursor:"pointer",
                      background: role===r.key ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.05)",
                      border: `1.5px solid ${role===r.key ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.08)"}`,
                    ...
                    <span style={{fontSize:11,fontWeight: role===r.key ? 700 : 500,color: role===r.key ? "#fff" : "rgba(255,255,255,0.55)",fontFamily:"'Outfit',sans-serif",textAlign:"center",lineHeight:1.2}}>{r.label}</span>
            ...
            {/* Guidelines */}
            <div style={{position:"relative",zIndex:1}}>
              <p style={{fontSize:10,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"rgba(255,255,255,0.35)",fontFamily:"'Outfit',sans-serif",marginBottom:8}}>Guidelines</p>
              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                {[
                  ...
                  <div key={i} style={{display:"flex",alignItems:"flex-start",gap:7}}>
                    <div style={{width:16,height:16,borderRadius:"50%",background:"rgba(16,185,129,0.2)",border:"1px solid rgba(16,185,129,0.4)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>
                      <span style={{fontSize:8,fontWeight:800,color:"#10B981"}}>{i+1}</span>
                    </div>
                    <span style={{fontSize:11,color:"rgba(255,255,255,0.55)",lineHeight:1.5}}>{g}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Field icons */}
            <div style={{position:"relative",zIndex:1}}>
              <p style={{fontSize:10,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"rgba(255,255,255,0.35)",fontFamily:"'Outfit',sans-serif",marginBottom:8}}>What you will need</p>
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {[
                  ...
                  <div key={item.tip} title={item.tip} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                    ...
                    <span style={{fontSize:9.5,color:"rgba(255,255,255,0.45)",fontWeight:500,textAlign:"center"}}>{item.tip}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Bottom */}
            <div style={{position:"relative",zIndex:1,borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:12}}>
              <p style={{fontSize:11,color:"rgba(255,255,255,0.4)",marginBottom:8}}>Already have an account?</p>
              <a href="/login" style={{display:"inline-flex",alignItems:"center",gap:7,padding:"8px 14px",borderRadius:9,border:"1.5px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.07)",color:"#fff",fontSize:12,fontWeight:600,textDecoration:"none",fontFamily:"'Outfit',sans-serif"}}>
          ...
          <div className="rp">
            ...
            <div style={{position:"sticky",bottom:0,background:"#F8FAFC",paddingTop:8,paddingBottom:8,marginTop:4,zIndex:10}}>
