...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { rows: tses } = await query(`
      SELECT
        te.employee_id AS tse_id, te.employee_code AS tse_code, tu.full_name AS tse_name,
        -- TSC attendance today
        (SELECT check_in_time FROM attendance a WHERE a.employee_id=te.employee_id AND a.attendance_date=CURRENT_DATE LIMIT 1) AS tse_check_in,
        ...
        (SELECT COUNT(*) FROM employee_store_assignments esa
         ...
         WHERE esa.tse_employee_id=te.employee_id AND esa.is_active=TRUE) AS total_bas,
        -- BAs present today
        (SELECT COUNT(*) FROM employee_store_assignments esa
         JOIN attendance a ON a.employee_id=esa.employee_id AND a.attendance_date=CURRENT_DATE
         WHERE esa.tse_employee_id=te.employee_id AND esa.is_active=TRUE) AS present_bas,
        -- LD sales
        (SELECT COALESCE(SUM(si.total_amount),0) FROM employee_store_assignments esa
         ...
         WHERE esa.tse_employee_id=te.employee_id AND esa.is_active=TRUE) AS ld_sales,
        -- MTD sales
        (SELECT COALESCE(SUM(si.total_amount),0) FROM employee_store_assignments esa
         JOIN sales_entries se ON se.employee_id=esa.employee_id AND se.sales_date >= $2
         JOIN sales_entry_items si ON si.sales_entry_id=se.sales_entry_id
         WHERE esa.tse_employee_id=te.employee_id AND esa.is_active=TRUE) AS mtd_sales
      FROM am_tse_assignments ata
      JOIN gm_employees te ON ata.tse_employee_id = te.employee_id
    ...
    const { rows: bas } = await query(`
      ...
      FROM am_tse_assignments ata
      JOIN employee_store_assignments esa ON esa.tse_employee_id=ata.tse_employee_id AND esa.is_active=TRUE
      ...
      LEFT JOIN gm_employees te ON esa.tse_employee_id=te.employee_id
      LEFT JOIN gm_users tu ON te.user_id=tu.user_id
      WHERE ata.am_employee_id = $1 AND ata.is_active=TRUE AND u.is_active=TRUE
      ORDER BY mtd_sales DESC
    `, [employeeId, monthStart])

    const totalTSCs    = tses.length
    const presentTSCs  = tses.filter((t: any) => t.tse_check_in).length
